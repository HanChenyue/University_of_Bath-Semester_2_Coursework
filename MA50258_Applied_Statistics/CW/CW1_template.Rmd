...
# Write formula_tom_q9 to csv
write.csv(formula_tom_q9, "formula_tom_q9.csv", row.names = FALSE)
...
pred_grid_q9_lm <-  formula_tom_q9 %>%
  data_grid(
    ...
    log10_championship_points = seq(min(formula_tom_q9$log10_championship_points), max(formula_tom_q9$log10_championship_points)))) %>% 
  add_predictions(lm_model_q9,
                  var = "race_time",
                  type = "response") %>%
  mutate(
