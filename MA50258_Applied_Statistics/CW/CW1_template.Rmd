...
GPT4 and CoPilot used for code and discussion suggestions, debugging purpose, and to check the correctness of the code.
...
where $\alpha_0$ is the intercept and $\alpha_1$ is the slope of the line. The estimated values of $\alpha_0$ and $\alpha_1$ are 3.48 and 0.96, respectively. This means that the expected value of `q3` increases by 0.96 seconds for every one-second increase in `q1`.
...
Looking only on the variable `log10(championship_points)`, the expected value of `q3` increases by $log_{10}(2) \times (\delta_2 + \delta_3) = log_{10}(2) \times (1.51 + (-0.03)) = 0.46$ for every twofold increase in `championship_points`.

The linear model and loess capture the relationship between `q1`, `championship_points`, and `q3` by considering the interaction between `q1` and `championship_points`. The models provided a good fit to the data for different mid-deciles of `championship_points` to provide us a comprehensive understanding of how `q1` and `championship_points` influence `q3`. 

However, when looking at the LOESS plot, we observed that the linear model fail to capture a fact that competitors with a higher `championship_points` tend to have faster lap times in the 3rd qualifying session when `q1` is approximately more than 100s, but it is very minor. When `q1` > 100, the effect of `championship_points` on `q3` became slight more pronounced in the LOESS fit showing us that competitors with higher `championship_points` will have a more pronounced edge by having better performance in `q3` relative to the performer having average `championship_points` (as shown in the zoom in plot below). This indicate that `championship_points` variable might have a slight effect on the relationship between `q1` and `q3` as captured by the LOESS model.

In other words, assuming that all competitors did 'badly' on the 1st qualifying session (when `q1` is approximately more than 100s), we expect that the competitors with higher `championship_points` will have a slight edge in the 3rd qualifying session relative to the competitors with lower `championship_points`.
...
ggplot(
  ...
  geom_vline(xintercept = intersection_q1, linetype = "dotted", color = "orange") +
  labs(
  ...
  scale_x_continuous(labels = function(x) paste0(x, "s")) +
  scale_y_continuous(labels = function(y) paste0(y, "s")) +
    annotate("text", x = Inf, y = Inf, label = "Theoretical Line, y = x", hjust = 1.1, vjust = 2, color = "black")
```
...
The modified linear model with the log-transformed `race_time` and `championship_points` would change the interpretation of the model coefficients. The log-transformed variables would allow for a non-linear relationship between the variables and the outcome variable, providing a more flexible model fit. The interpretation of the coefficients would be in terms of the percentage change in the outcome variable for a one-unit change in the predictor variables. The log-transformed variables would also help to stabilise the variance of the residuals and improve the model's performance (Generally, in a linear model, the more variables we consider, the less the variance will be). The log-transformed model would provide a better fit to the data and capture the non-linear relationship between the variables more accurately.
...
Based on the plots, we can see significant variations in the relationship between `q1`, `log10(championship_points)`, and `race_time` across different circuits. Given this observed variability, it is evident that a one-size-fits-all model may not adequately capture the nuances of each circuit. Therefore, GAM models that allow for non-linear relationships between the variables may provide a better fit to this datasets due to its flexibility in capturing the non-linear relationships between the variables.

From the plots below, we can observed that each circuit displays different performance characteristics. For example, the Japanese Grand Prix circuit has less variance compared to the other four circuits. Thus, the impact of the circuit on the relationship between `q1`, `log10(championship_points)`, and `race_time` is significant and should be considered when modelling the data.
