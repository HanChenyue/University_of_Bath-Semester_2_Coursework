...
```{r, message=FALSE}
# Filtering out an extreme outlier on datapoint no. 2568 that has a lap time of 16min, this is done by ignoring all datapoint of more than 180s (3min)
...
formula_tom_no_na <- formula_tom %>% drop_na(q1_in_seconds, q3_in_seconds) %>% filter(q1_in_seconds < 180)
ggplot(
  formula_tom_no_na,
  aes(x = q1_in_seconds, y = q3_in_seconds)) +
  geom_point(alpha = 0.25) +
  ...
  labs(
    ...
    x = "q1 (seconds)",
    y = "q3 (seconds)",
    subtitle = "Excluding missing data and an extreme outlier"
  ) +
  theme(legend.position = "bottom") +
  scale_color_manual(
    name = "Fits",
