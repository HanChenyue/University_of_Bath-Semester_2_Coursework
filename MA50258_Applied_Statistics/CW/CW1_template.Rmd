...
lm_summary <- broom::tidy(lm_fit) %>% as.data.frame()#%>% select(term, estimate)

# Rename the rows and columns of the data frame
lm_summary_custom <- data.frame(
  term = c("Intercept", "q1"),
...
```{r}
# mid_decile_points_q12 <- quantile(formula_tom_q12$log10_championship_points, probs = seq(0.05, 0.95, by = 0.1))
# mid_decile_points_q12 <- signif(mid_decile_points, digits = 3)
# kable(mid_decile_points_q12, caption = "Mid-Decile Points", align = "c")
# 
# lm_pred_grid_q12 <-  formula_tom_q12 %>%
#   data_grid(
#     log10_championship_points = mid_decile_points_q12,
#     circuit_ID = unique(formula_tom_q12$circuit_ID),
#     q1_in_seconds = seq(min(formula_tom_q12$q1_in_seconds), max(formula_tom_q12$q1_in_seconds))) %>%
#   add_predictions(
#     lm_model_q12,
#     var = "race_time",
#     type = "response") %>% 
...
# Generate a color for each circuit_ID
# circuit_id_colors <- viridis::viridis(length(unique(formula_tom_q12$circuit_ID)))

# Generate a color for each level of log10_championship_points
# mid_decile_colors <- viridis::viridis(length(unique(lm_pred_grid_q12$log10_championship_points)))
...
#     aes(x = q1_in_seconds, y = race_time, color = circuit_ID),
#     alpha = 0.25
#   ) +
#   geom_line(
#     data = lm_pred_grid_q12,
#     aes(x = q1_in_seconds, y = race_time, color = circuit_ID))
...
# pred_grid_for_q12 <- formula_tom_q12 %>% 
#   data_grid(
#     q1_in_seconds = seq(min(formula_tom_q12$q1_in_seconds), max(formula_tom_q12$q1_in_seconds)),
#     log10_championship_points = seq(min(formula_tom_q12$log10_championship_points), max(formula_tom_q12$log10_championship_points)),
#     circuit_ID = unique(formula_tom_q12$circuit_ID)) %>% 
#   add_predictions(
#     gam_model_q12,
#     var = "race_time",
