...
# formula_tom_no_na_q4 <- formula_tom %>% drop_na(championship_points, q1_in_seconds, q3_in_seconds) %>% filter(q1_in_seconds < 180)
formula_test <- formula_tom %>% drop_na(championship_points, q1_in_seconds, q3_in_seconds) %>%  filter(q1_in_seconds < 180)
log10_championship_points <- log10(formula_test$championship_points + 1)
ggplot(
  formula_test,
...
formula_test_break_points <- formula_test %>%
...
# Ignore all data points with missing values
formula_tom_no_na_q4 <- formula_tom %>% drop_na(championship_points, q1_in_seconds, q3_in_seconds) # %>% filter(q1_in_seconds < 180)

# Transform the championship_points variable to log10 scale
championship_points_in_log10 <- log10(formula_tom_no_na_q4$championship_points + 1)

# Plot the LOESS fit
ggplot(
  formula_tom_no_na_q4,
  # aes(x = q1_in_seconds + q1_in_seconds*championship_points_in_log10, y = q3_in_seconds)) +
  # aes(x = q1_in_seconds + q1_in_seconds*formula_tom_no_na_q4$championship_points, y = q3_in_seconds)) +
  aes(x = q1_in_seconds, y = q3_in_seconds, group = championship_points_in_log10)) +
  geom_point(alpha = 0.25) +
  geom_smooth(aes(col = "loess smoothing"), method = "loess", se = F, linewidth = 0.5) +
  labs(
    title = "LOESS on q3 against q1 with non log-transformed championship points",
    x = "q1 (seconds)",
    y = "q3 (seconds)",
    subtitle = "Excluded missing data"
  ) +
...
ggplot(
  formula_tom_no_na_q4,
  aes(x = q1_in_seconds*championship_points_in_log10, y = q3_in_seconds)) +
  geom_point(alpha = 0.25) +
  geom_smooth(aes(col = "loess smoothing"), method = "loess", se = F, linewidth = 0.5) +
  labs(
    title = "loess fit of q3 against q1",
    x = "q1 (seconds)",
    y = "q3 (seconds)",
...
```{r,message=FALSE}
# Your code answer here
qmin <- pmin(formula_tom$q1_in_seconds, formula_tom$q2_in_seconds, formula_tom$q3_in_seconds)
```
