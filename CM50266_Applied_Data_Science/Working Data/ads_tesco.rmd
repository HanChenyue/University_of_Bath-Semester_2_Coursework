...
ggplot(income_data, aes(x = year, y = mean_income_pound)) +
  geom_point() +
...
ggplot(income_data, aes(x = year, y = median_income_pound)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, colour = "red") +
  geom_smooth(method = "loess", se = FALSE, colour = "blue") +
  theme_minimal() +
...
The correlations matches research outcomes of existing literature on the relationship between income and food choices, where higher-income individuals tend to prioritize healthier and more expensive food items, while lower-income individuals may opt for more affordable and less nutritious options:
...
Let's determine the strength of the relationship between income levels and food purchase categories while controlling for other variables. A multiple regression could be particularly insightful if you suspect that other demographic or regional factors may influence purchasing patterns.
```{r}
