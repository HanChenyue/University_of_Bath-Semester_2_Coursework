...
columns <- c("f_beer", "f_dairy", "f_eggs", "f_fats_oils", "f_fish",
             "f_fruit_veg", "f_grains", "f_meat_red", "f_poultry",
             "f_readymade", "f_sauces", "f_soft_drinks", "f_spirits",
             "f_sweets", "f_tea_coffee", "f_water", "f_wine")
...
for (mon in months) {
  file_name <- paste0(mon, "_borough_grocery.csv")
  borough_data[[mon]] <- read.csv(file_name, check.names = FALSE)
}

# Convert the list to a data frame
df_borough <- bind_rows(borough_data, .id = "Month")
df_borough$Month <- factor(df_borough$Month, levels = months)

# Compute the monthly averages for each category
monthly_averages <- df_borough %>%
  group_by(Month) %>%
  summarise(across(all_of(columns), ~ mean(.x, na.rm = TRUE))) %>%
  pivot_longer(-Month, names_to = "Category", values_to = "Average")



# Plotting with ggplot2 using scale_color_manual to change labels
# Define the new labels for the categories
# Update the category names directly in the data frame
monthly_averages$Category <- factor(monthly_averages$Category,
                                    levels = c("f_beer", "f_dairy", "f_eggs", "f_fats_oils", "f_fish",
                                               "f_fruit_veg", "f_grains", "f_meat_red", "f_poultry",
                                               "f_readymade", "f_sauces", "f_soft_drinks", "f_spirits",
                                               "f_sweets", "f_tea_coffee", "f_water", "f_wine"),
                                    labels = c("Beer", "Dairy", "Eggs", "Fats & Oils", "Fish",
                                               "Fruit & Veg", "Grains", "Red Meat", "Poultry",
                                               "Ready Made", "Sauces", "Soft Drink", "Spirits",
                                               "Sweets", "Tea & Coffee", "Water", "Wine"))

# Now, when you plot, the facet labels will use the updated names
overall_monthly_sales <- ggplot(monthly_averages, aes(x = Month, y = Average, group = Category, color = Category)) +
  geom_line() +
  facet_wrap(~ Category, scales = 'free_y', ncol = 4) +
  theme_minimal(base_size = 18) +
  labs(title = "Monthly Average Sales by Category",
       x = "Month",
       y = "Average Sales") +
  scale_color_manual(values = rainbow(length(unique(monthly_averages$Category))), 
                     labels = levels(monthly_averages$Category)) +
  theme(legend.position = "left") # Place the legend at the bottom

overall_monthly_sales
# Show the plot
# ggsave("monthly_averages_by_category.png", width = 38, height = 19)

```


```{r}
# Use k-means cluster analysis to identify possible purchasing pattern worth exploring
...
purchasing_patterns_cluster_function <- function(data, hand_picked_features){
  ...
  features <- data %>% dplyr::select(hand_picked_features, 'f_beer', 'f_dairy', 'f_eggs', 'f_fats_oils', 'f_fish', 'f_fruit_veg', 'f_grains',
  ...
  fviz_nbclust(scaled_data, kmeans, method = "wss") + labs(title = 'Elbow Method') + geom_line(colour = "red", size = 1.2)
}
```
...
- We will then investigate the purchasing patterns using normalisation to understand the preferences of each group to produce insights from this approach.
...
#          cl.cex = 0.7, # Colour legend text size
...
# Correlation plot between gender and purchasing (borough level)
# Select relevant columns
borough_year_gender_cor <- borough_year[c('male', 'female', 'f_beer', 'f_dairy', 'f_eggs', 'f_fats_oils', 'f_fish', 'f_fruit_veg', 'f_grains',
                'f_meat_red', 'f_poultry', 'f_readymade', 'f_sauces', 'f_soft_drinks', 'f_spirits',
                'f_sweets', 'f_tea_coffee', 'f_water', 'f_wine')]
# Define new names for the columns
borough_year_gender_cor <- borough_year_gender_cor %>% rename(
  Male = 'male',
  Female = 'female',
  Beer = 'f_beer',
  Dairy = 'f_dairy',
  Eggs = 'f_eggs',
  `Fats and Oils` = 'f_fats_oils',
  Fish = 'f_fish',
  `Fruits & Vegetables` = 'f_fruit_veg',
  Grains = 'f_grains',
  `Red Meat` = 'f_meat_red',
  Poultry = 'f_poultry',
  `Ready Made` = 'f_readymade',
  Sauces = 'f_sauces',
  `Soft Drinks` = 'f_soft_drinks',
  Spirits = 'f_spirits',
  Sweets = 'f_sweets',
  `Tea & Coffee` = 'f_tea_coffee',
  Water = 'f_water',
  Wine = 'f_wine'
)
borough_year_gender_cor_matrix <- cor(borough_year_gender_cor, use = "pairwise.complete.obs", method = "pearson")
corrplot(borough_year_gender_cor_matrix, method = "color",
         type = "upper", # Only upper triangular part of the matrix
         order = "hclust", # Hierarchical clustering order
         tl.col = "black", # Text label colour
         tl.srt = 45, # Text label rotation
         ...
         addCoef.col = "black", # Add coefficient colour
         title = "Correlation Matrix of Food Purchases and Gender",
         cl.cex = 0.9, # Colour legend text size
         number.cex = 0.9, # Correlation coefficient text size
         number.digits = 1, # Number of digits in correlation coefficient
         mar = c(0, 0, 1, 0), # Margins around the plot
         col = colorRampPalette(c("blue", "white", "red"))(1000)) # Change colour scheme

# For age group (borough level)
borough_year_age_cor <- borough_year[c('age_0_17', 'age_18_64', 'age_65+', 'f_beer', 'f_dairy', 'f_eggs', 'f_fats_oils', 'f_fish', 'f_fruit_veg', 'f_grains',
                'f_meat_red', 'f_poultry', 'f_readymade', 'f_sauces', 'f_soft_drinks', 'f_spirits',
                'f_sweets', 'f_tea_coffee', 'f_water', 'f_wine')]
# Define new names for the columns
borough_year_age_cor <- borough_year_age_cor %>% rename(
  `Age 0-17` = 'age_0_17',
  `Age 18-64` = 'age_18_64',
  `Age 65+` = 'age_65+',
  Beer = 'f_beer',
...
borough_year_age_cor_matrix <- cor(borough_year_age_cor, use = "pairwise.complete.obs", method = "pearson")

corrplot(borough_year_age_cor_matrix, method = "color",
         type = "upper", # Only upper triangular part of the matrix
         order = "hclust", # Hierarchical clustering order
         tl.col = "black", # Text label colour
         tl.srt = 45, # Text label rotation
         tl.cex = 0.9, # Text label size
         addCoef.col = "black", # Add coefficient colour
         title = "Correlation Matrix of Food Purchases and Age Groups",
         cl.cex = 0.9, # Colour legend text size
         number.cex = 0.9, # Correlation coefficient text size
         number.digits = 1, # Number of digits in correlation coefficient
         mar = c(0, 0, 1, 0), # Margins around the plot
...
- Area_sq_km: Weak negative correlation with weight (-0.18) and weak positive correlation with volume (0.08), indicating that the size of the borough is also affected by income as well (center of London may be small but a lot of economic activity happen there, and a generally higher income population)
...
ggplot(borough_year, aes(x = avg_age, y = weight)) +
  geom_point(alpha = 0.6, size = 3) +
  geom_smooth(method = "loess", se = FALSE, color = "red", size = 3) +
...
ggplot(osward_year, aes(x = avg_age, y = weight)) +
  geom_point(alpha = 0.6, size = 3) +
  geom_smooth(method = "loess", se = FALSE, color = "orange", size = 3) +
...
ggplot(msoa_year, aes(x = avg_age, y = weight)) +
  geom_point(alpha = 0.6, size = 3) +
  geom_smooth(method = "loess", se = FALSE, color = "purple", size = 3) +
...
ggplot(lsoa_year, aes(x = avg_age, y = weight)) +
  geom_point(alpha = 0.6, size = 3) +
  geom_smooth(method = "loess", se = FALSE, color = "blue", size = 3) +
...
calculate_and_plot_purchases <- function(data, product_categories, age_columns) {
  ...
  normalized_purchases <- lapply(names(purchase_sums_by_age), function(age) {
    ...
    new_product_labels <- setNames(
    c("Beer", "Dairy", "Eggs", "Fats & Oils", "Fish", "Fruit & Veg", "Grains", "Red Meat", "Poultry", "Readymade", "Sauces", "Soft Drinks", "Spirits", "Sweets", "Tea & Coffee", "Water", "Wine"), 
    product_categories
  )
  ...
  #   ggtitle("Normalised Purchasing Patterns by Age Group Across Product Categories") 
  # print(melted_data_age)
}
...
#   # Ensure that all necessary columns are present in the data
#   required_columns <- c(product_categories, age_columns, "population")
#   missing_columns <- setdiff(required_columns, names(data))
#   if (length(missing_columns) > 0) {
#     stop("The following required columns are missing from the dataframe: ", paste(missing_columns, collapse = ", "))
#   }
#   
#   # Prepare data for ANOVA by creating a long format DataFrame where each row represents a single observation
#   long_format_data <- data %>%
#     pivot_longer(cols = all_of(product_categories), names_to = "Category", values_to = "Fraction") %>%
#     pivot_longer(cols = all_of(age_columns), names_to = "Age_Group", values_to = "Age_Fraction") %>%
#     mutate(Age_Fraction = Fraction * Age_Group / population)
#     # Removed the problematic select statement
# 
#   # Perform ANOVA for each category and store results in a list
#   results <- list()
#   for (category in product_categories) {
#     category_data <- filter(long_format_data, Category == category)
# 
#     # Check for sufficient data to perform ANOVA
#     if (n_distinct(category_data$Age_Group) < 2) {
#       results[[category]] <- list(ANOVA = "Not enough data to perform ANOVA",
#                                   Tukey = "ANOVA not performed")
#       next
#     }
# 
#     # Perform ANOVA
#     anova_result <- aov(Age_Fraction ~ Age_Group, data = category_data)
#     anova_summary <- summary(anova_result)
#     
#     # Perform Tukey's HSD test if ANOVA is significant
#     if (anova_summary[[1]]$'Pr(>F)'[[1]] < 0.05) {
#       tukey_result <- TukeyHSD(anova_result)
#       results[[category]] <- list(ANOVA = anova_summary, Tukey = tukey_result)
#     } else {
#       results[[category]] <- list(ANOVA = anova_summary, Tukey = "ANOVA not significant, no post hoc test performed")
#     }
#   }
# 
#   return(results)
...
# 
# # Review results
# product_categories <- c("f_beer", "f_dairy", "f_eggs", "f_fats_oils", "f_fish", "f_fruit_veg", "f_grains", "f_meat_red", "f_poultry", "f_readymade", "f_sauces", "f_soft_drinks", "f_spirits", "f_sweets", "f_tea_coffee", "f_water", "f_wine")
# age_columns <- c("age_0_17", "age_18_64", "age_65+")
# results_borough <- perform_anova_tukey(borough_year, product_categories, age_columns)
...
calculate_and_plot_purchases <- function(data, product_categories, age_columns) {
  ...
  #   theme_minimal() +
  #   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size = 6)) +
  #   labs(x = "Product Categories", y = "Normalised Fraction of Purchases", fill = "Age Group") +
  #   ggtitle("Normalised Purchasing Patterns by Age Group Across Product Categories") +
  #   
  #   guides(fill = FALSE)
    ggplot(melted_data_age, aes(x = Product, y = Fraction, fill = Age_Group)) +
...
# ggsave("age_group_purchase.png", width = 10, height = 10)
```
```{r}
# new_product_labels <- setNames(c("Beer", "Dairy", "Eggs", "Fats & Oils", "Fish", "Fruit & Veg", "Grains", "Red Meat", "Poultry", "Ready Made", "Sauces", "Soft Drinks", "Spirits", "Sweets", "Tea & Coffee", "Water", "Wine"), product_categories)
...
#   
#   rownames(normalized_purchases_gender_df) <- new_product_labels[rownames(normalized_purchases_gender_df)]
#   
#   # Plotting the data
...
```{r}
# Alternate code: Testing
calculate_and_plot_purchases_gender <- function(data, product_categories) {
  # # Ensure product categories are unique
  # if(length(unique(product_categories)) != length(product_categories)) {
  #   stop("product_categories contains duplicates")
  # }
  
  # Create new product labels mapping
  new_product_labels <- setNames(
    c("Beer", "Dairy", "Eggs", "Fats & Oils", "Fish", "Fruit & Veg", "Grains", "Red Meat", "Poultry", "Readymade", "Sauces", "Soft Drinks", "Spirits", "Sweets", "Tea & Coffee", "Water", "Wine"), 
    product_categories
  )
  ...
  #   theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  #   labs(title = "Normalized Purchasing Patterns by Gender Across Product Categories",
  #        x = "Product Categories", y = "Normalised Fraction of Purchases",
  #        fill = "Gender") +
    ggplot(melted_data_gender, aes(x = Product, y = Fraction, fill = Gender)) +
    geom_bar(stat = "identity", position = position_dodge()) +
    scale_fill_manual(values = c("male" = "#25A795", "female" = "#EB472F"), ##754C8F
                      labels = c("male" = "Male", "female" = "Female")) +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    labs(title = "Normalised Purchasing Patterns by Gender Across Product Categories",
         x = "Product Categories", y = "Normalised Fraction of Purchases",
         fill = "Gender")
}

# Make sure to pass the unique set of product categories to your function
calculate_and_plot_purchases_gender(borough_year, product_categories)
```
...
calculate_and_plot_purchases <- function(data, product_categories, age_columns) {
  ...
  rownames(normalized_purchases_df) <- product_categories
    ...
    rownames(normalized_purchases_df) <- new_product_labels[rownames(normalized_purchases_df)]
  normalized_purchases_df <- normalized_purchases_df %>%
    tibble::rownames_to_column(var = "Product")
  ...
  # return(melted_data_age)
  melted_data_age

  # ggplot(melted_data_age, aes(x = Product, y = Fraction, fill = Age_Group)) +
  #   geom_bar(stat = "identity", position = position_dodge()) +
  #   scale_fill_manual(values = c("age_0_17" = "#25A795", "age_18_64" = "#754C8F", "age_65+" = "#EB472F"),
  #                     labels = c("age_0_17" = "0-17", "age_18_64" = "18-64", "age_65+" = "65+")) +
  #   theme_minimal() +
  #   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size = 6)) +
  #   labs(x = "Product Categories", y = "Normalised Fraction of Purchases", fill = "Age Group") +
  #   ggtitle("Normalised Purchasing Patterns by Age Group Across Product Categories")
...
#     pivot_longer(cols = -Product, names_to = "Age_Group", values_to = "Fraction")
# 
#   # Factorize the Age_Group to ensure that factor levels are consistent
#   melted_data_age$Age_Group <- factor(melted_data_age$Age_Group, levels = age_columns)
# # Define colors for the age groups
# age_colors <- setNames(
#   c("#25A795", "#754C8F", "#EB472F"), 
#   age_columns
# )
# 
...
# )
# 
# # Now use the named vectors in the scale_fill_manual
# ggplot(melted_data_age, aes(x = Product, y = Fraction, fill = Age_Group)) +
#   geom_bar(stat = "identity", position = position_dodge()) +
...
# }
# 
# # Ensure that 'borough_year', 'product_categories', and 'age_columns' are defined in your environment
# calculate_and_plot_purchases(borough_year, new_product_labels, age_columns)
```

```{r}
# # Assuming that 'borough_year' is your data frame, 'product_categories' is a vector of column names for the products,
# # and 'age_columns' is a vector of column names for the age groups in 'borough_year'.
# 
# # Define the new product labels
# new_product_labels <- c("Beer", "Dairy", "Eggs", "Fats & Oils", "Fish", "Fruit & Veg", "Grains", "Red Meat", "Poultry", "Readymade", "Sauces", "Soft Drinks", "Spirits", "Sweets", "Tea & Coffee", "Water", "Wine")
# 
# # Ensure the 'product_categories' vector has the correct product column names
# product_categories <- intersect(new_product_labels, colnames(borough_year))
# 
# # Calculate the sum of purchases for each product category by age group
# purchase_sums_by_age <- lapply(age_columns, function(age) {
#   colSums(borough_year[product_categories] * borough_year[[age]], na.rm = TRUE)
# })
# names(purchase_sums_by_age) <- age_columns
# 
# # Normalize these sums by the total count for each age group
# normalized_purchases <- lapply(age_columns, function(age) {
#   purchase_sums_by_age[[age]] / sum(borough_year[[age]], na.rm = TRUE)
# })
# 
# # Create a combined normalized purchases data frame with product categories as row names
# normalized_purchases_df <- as.data.frame(normalized_purchases)
# rownames(normalized_purchases_df) <- product_categories
# 
# # Convert to long format
# melted_data_age <- normalized_purchases_df %>%
#   tidyr::pivot_longer(cols = everything(), names_to = "Product", values_to = "Fraction")
# 
# # Merge with the original data to get the age groups
# borough_year_long <- borough_year %>%
#   tidyr::pivot_longer(cols = age_columns, names_to = "Age_Group", values_to = "Value") %>%
#   dplyr::select(-c(product_categories))  # Remove product columns
# 
# # Combine both long format data
# combined_data <- merge(melted_data_age, borough_year_long, by = "Product")
# 
# # Check for NAs in Age_Group
# print(any(is.na(combined_data$Age_Group)))
# 
# # Plotting the data
# ggplot(combined_data, aes(x = Product, y = Fraction, fill = Age_Group)) +
#   geom_bar(stat = "identity", position = position_dodge()) +
#   scale_fill_manual(values = c("age_0_17" = "#25A795", "age_18_64" = "#754C8F", "age_65+" = "#EB472F")) +
#   theme_minimal() +
#   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size = 6)) +
...
ggplot(income_data, aes(x = year, y = mean_income_pound, colour = area_id_name)) +
  ...
  # geom_point() +
    theme(
    axis.title.x = element_text(size = 24), # Adjust x-axis title size
    axis.title.y = element_text(size = 24), # Adjust y-axis title size
    axis.text.x = element_text(size = 22),  # Adjust x-axis text size
...
ggplot(income_data, aes(x = year, y = mean_income_pound, colour = area_id_name)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, colour = "red", alpha = 0.3) +
...
top_3_mean_income_2015 <- income_data_2015 %>%
  dplyr::select(area_id, area_id_name, mean_income_pound) %>% 
  top_n(5, mean_income_pound) %>% 
  arrange(desc(mean_income_pound))

top_3_median_income_2015 <- income_data_2015 %>%
  dplyr::select(area_id, area_id_name, median_income_pound) %>% 
  top_n(5, median_income_pound) %>% 
  arrange(desc(median_income_pound))
...
kable(top_3_mean_income_2015, caption = "Top 5 Boroughs with Highest Mean Income in 2015") %>% 
  kable_styling("striped", full_width = FALSE, position = "center")

kable(top_3_median_income_2015, caption = "Top 5 Boroughs with Highest Median Income in 2015") %>%
  kable_styling("striped", full_width = FALSE, position = "center")

kable(bottom_2_mean_income_2015, caption = "Bottom 5 Boroughs with Lowest Mean Income in 2015") %>%
  kable_styling("striped", full_width = FALSE, position = "center")

kable(bottom_2_median_income_2015, caption = "Bottom 5 Boroughs with Lowest Median Income in 2015") %>%
  kable_styling("striped", full_width = FALSE, position = "center")
```
...
corrplot(income_food_correlation_matrix, method = "color", 
         ...
         tl.col = "black", # Text label color
         tl.srt = 45, # Text label rotation
         tl.cex = 0.9, # Reduce text label size for space
         addCoef.col = "black", # Add coefficient color
         title = "Correlation Matrix of Income and Food Category Purchases",
         cl.cex = 0.9, # Color legend text size
         number.cex = 0.9, # Reduce correlation coefficient text size
         number.digits = 2, # Number of digits in correlation coefficient
         sig.level = 0.05, # Only show significant correlations
         diag = FALSE, # Do not show diagonal
         mar = c(0, 0, 1, 0), # Margins around the plot
...
```{r, fig.width=18, fig.height=18}
old_names <- c('mean_income_pound', 'median_income_pound', product_categories)
new_names <- c("Mean Income", "Median Income", "Beer", "Dairy", "Eggs", "Fats and Oils", "Fish", 
               "Fruits and Vegetables", "Grains", "Red Meat", "Poultry", "Ready Made", "Sauces", 
               "Soft Drinks", "Spirits", "Sweets", "Tea and Coffee", "Water", "Wine")

# Now you need to ensure that the length of old_names and new_names is the same and they correspond correctly
# The old names should match exactly with column names in your data frame
# Make sure 'borough_with_2015_income_data' is the correct data frame

# Calculate the correlation matrix
correlation_matrix <- cor(borough_with_2015_income_data[, old_names], use = "pairwise.complete.obs")

# Now, this is important: assign the new names to the rows and columns of the correlation matrix
rownames(correlation_matrix) <- new_names
colnames(correlation_matrix) <- new_names

# Generate the corrplot
corrplot(
  correlation_matrix, # Make sure to use the variable that has the correlation matrix
  method = "color",
  type = "upper",
  order = "hclust",
  tl.col = "black",
...
# Inspect the structure of the data to confirm column names
# str(borough_year)

# Assuming 'age_columns' are the correct age group columns in 'borough_year'
# and 'product_categories' are the product columns.

# Normalizing purchases for each product within each age group
normalized_purchases <- borough_year %>%
  mutate(across(all_of(age_columns), ~ ./sum(.x, na.rm = TRUE))) %>%
  select(all_of(c(product_categories, age_columns)))

# Check the normalization result
# head(normalized_purchases)

# Reshape the normalized data to a long format
melted_data <- normalized_purchases %>%
  pivot_longer(cols = all_of(age_columns), names_to = "Age_Group", values_to = "Value") %>%
  pivot_longer(cols = all_of(product_categories), names_to = "Product", values_to = "Fraction")

# Check the reshaped data
head(melted_data)

# Now plot
ggplot(melted_data, aes(x = Product, y = Fraction, fill = Age_Group)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  scale_fill_manual(values = c("#25A795", "#754C8F", "#EB472F")) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(x = "Product Categories", y = "Normalized Fraction of Purchases", fill = "Age Group") +
  ggtitle("Normalized Purchasing Patterns by Age Group Across Product Categories")
