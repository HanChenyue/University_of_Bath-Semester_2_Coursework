...
# Reading and processing data for each month
for (mon in months) {
  item_aver <- c()
  file_path <- paste0("./7796666/", mon, "_borough_grocery.csv")
  data <- read_csv(file_path)
  
  for (col in columns) {
    mon_aver <- mean(data[[col]], na.rm = TRUE)
    item_aver <- c(item_aver, mon_aver)
  }
  
  sales_data[[mon]] <- item_aver
}

# Transpose sales_data
sales_data_transposed <- as.data.frame(do.call(rbind, sales_data))

# Calculating year average
y_average <- c()
for (col in columns) {
  aver <- mean(sales_data_transposed[[col]], na.rm = TRUE)
  y_average <- c(y_average, aver)
}

# Checking that each category has a monthly average sales value
for (i in seq_along(columns)) {
  if (length(sales_data_transposed[[i]]) != length(months)) {
    message("Warning: ", columns[i], " does not have data for every month.")
  }
}

# Plotting the monthly average sales for each category
pdf("monthly_average_sales.pdf", width = 15, height = 20) # Saving the plot to a PDF
par(mfrow = c(6, 3), mar = c(4, 4, 2, 1)) # Set multi-plot layout

for (i in seq_along(columns)) {
  if (i <= length(columns)) {
    plot(months, sales_data_transposed[[i]], type = 'o', main = columns[i],
         xlab = 'Month', ylab = 'Year Average Sales', pch = 19, col = "blue")
    abline(h = y_average[i], col = 'red', lty = 'dashed', lwd = 2) # Add average line
  }
}
```
...
age_combined_plot <- ggarrange(plotlist = plots, ncol = 2, nrow = 2)
# You can add a common title with `annotate_figure` from `ggpubr`
age_combined_plot <- annotate_figure(age_combined_plot, 
                                 top = text_grob("Elbow Method Plots", face = "bold", size = 14))
```
```{r, WARNING=FALSE}
