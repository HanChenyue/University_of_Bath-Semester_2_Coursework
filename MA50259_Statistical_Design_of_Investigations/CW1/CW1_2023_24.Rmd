...
AI (RStudio built-in Co-Pilot and ChatGPT) are used in this coursework for code suggestions and debugging purposes.
...
linear trend due to varying the amount of baking powder used. Also, to simplify our calculations, instead of using contrast weights of $c = (0, -3, -1, 1, 3)$ on design matrix X, we can perform the test by using contrast weights of $c = (-3, -1, 1, 3)$ on matrix Z (cell means model).
...
contrast: $$c = (-3, -1, 1, 3)$$ which is a classical orthogonal
...
$c^T\hat{\tau_i} = \text{contrast weight} \times \text{mean within individual treatment levels} = (-3 \times 10.800) + (-1 \times 27.450) + (1 \times 46.850) + (3 \times 62.725) = 175.18$
...
i.  The errors $\epsilon_{ij}$ are independent and identically
    distributed $N(0,\sigma^2)$.

ii. The errors $\epsilon_{ij}$ are independent of the treatment effects
    $\tau_{i}$.
...
is a completely randomised design (CRD) and the errors $\epsilon_{ij}$
are assumed to be independent and identically distributed
$N(0,\sigma^2)$. The fifth assumption is justified because the variance
of the experimental error $\sigma^2$ is constant as shown in the ANOVA
table above.
...
10. If the dough were made in batches and the four replicate biscuit
    ...
    data be analyzed if this were the case?

\hrulefill

\underline{Answer:}

If the dough were made in batches and the four replicate biscuit rise
heights in each column were all from the same batch, then the assumption
of independence of the errors $\epsilon_{ij}$ would be violated. This is
because the errors $\epsilon_{ij}$ would not be independent of one
another. In this case, the data could be analysed using a randomised
complete block design (RCBD) where the blocks would be the batches of
dough and the treatments would be the levels of the baking powder. The
ANOVA table would be computed using the same method as in question 6 but
the design matrix $\boldsymbol{X}$ would be different.
