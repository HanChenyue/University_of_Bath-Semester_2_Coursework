...
```{r}
## OLD ANSWER - DELETE LATER
# # Compute the contrast matrix
# C <- c(0,1,-1,0,0) # Contrast to test the hypothesis that increase in rise height is a linear function of the increase in baking powder in the dough in this case it is 0.25 tsp - 0.5 tsp
# 
# # Compute the contrast estimate
ginverse <- rbind(0, cbind(0, solve(t(Z) %*% Z))) # Alternatively, use the MASS library but I'm not sure if that is allowed
# Beta <- ginverse %*% t(X) %*% y
# contrast_estimate <- t(C) %*% Beta
# contrast_estimate # The contrast estimate is -16.65

# ChatGPT code
# # Linear contrast including the intercept
linear_contrast <- c(0, -3, -1, 1, 3) # make c_0 = 0 
 
# # Applying the contrast
# Assuming 'biscuits$riseht' is your response variable and 'Z' is your design matrix
contrast_value <- sum(linear_contrast * colMeans(X * y))
# 
# # Computing the standard error of the contrast
# # This requires the standard error of the means, which can be calculated from the data
# # Assuming the variance of the errors is known or can be estimated
# # If unknown, it needs to be estimated from the data
error_variance <- var(y) # replace this with your estimate if available
# sample_size <- nrow(X) # or the number of observations per treatment if it differs
# std_error_contrast <- sqrt(sum((linear_contrast^2) * error_variance) / sample_size)
# 
# # Performing the t-test
# t_value <- contrast_value / std_error_contrast
# df <- nrow(X) - length(linear_contrast) # degrees of freedom
# p_value <- 2 * pt(-abs(t_value), df) # two-tailed p-value
# 
# # Result
# list(t_value = t_value, p_value = p_value)
...
\hrulefill
\underline{Answer:}
In the model for the crd, the statistical hypothesis of interest is $h_0$: $\mu_1$ = $\mu_2$ = ...= $\mu_t$ or $\tau_1$ = $\tau_2$ = ...= $\tau_t$ where $\mu_i$ is the mean response for the $i$th treatment and $\tau_i$ is the effect of the $i$th treatment. 

The alternative hypothesis is $H_a$: at least two of the $\tau$s differs.
...
# \hat{\sigma}^2 = SSE / (n - t)
# where SSE is the sum of squares of the experimental error and n is the total number of experimental units.
# The degrees of freedom for the error is n - t.

# Estimate the variance of the experimental error
# SSE_test <- (t(y) %*% y) - (t(Beta) %*% t(X) %*% y) # Non-reduced model
# SSE_test / (n - t)
# For non-reduced model
SSE_test2 <- t(y) %*% (diag(n) - X %*% ginverse %*% t(X)) %*% y
variance_of_experimental_error <- SSE_test2 / (n - t)
round(variance_of_experimental_error, 4) # round to 4 decimal places
