...
A (Quantile-Quantile) QQ plot can be used to check the whether the residuals are normally distributed (Gaussian). If the residuals are normally distributed, the points in the QQ plot will fall approximately along a straight line.
...
It would change the nature of the experiment as we lost the advantages gained due to randomisation. Randomisation is important as it helps to ensure that each individual treatment groups are comparable and that the effects observed are due to treatments rather than any underlying differences between the treatment groups. If the biscuits are made all from the same batches, this introduces the possibility of batch-to-batch variation influencing the results. It might have confound the effects of the baking powder, making it harder for us to isolate the true effect of each treatment level of the baking powder.
...
We would like to detect an absolute difference $\Delta=1$ with high power. The number of replicates $r$ necessary to achieve at least 90% power using a significance level $\alpha=0.05$ is $nrep = 4$. This is the minimum number of replicates required to achieve at least 90% power.
...
- For length:
  \[ \lambda_\text{length} = b \times r \times \sum_{i=1}^{a} \frac{(\mu_{\text{length},i} - \mu_{\text{overall}})^2}{\sigma^2} \]

- For width:
  \[ \lambda_\text{width} = a \times r \times \sum_{i=1}^{b} \frac{(\mu_{\text{width},i} - \mu_{\text{overall}})^2}{\sigma^2} \]
  where \( b \) is the number of levels in width, \( a \) is the number of levels in length, \( r \) is the number of replicates, \( \mu_{\text{width},i} \) is the mean for each level of width, and \( \mu_{\text{overall}} \) is the overall mean.
...
factor_combination <- data.frame(
  length = c(6, 6, 6, 5, 5, 6, 5, 5, 5, 5, 6, 6, 5, 6, 5, 6),
  width = c(5.5, 6.5, 5.5, 5.5, 5.5, 6.5, 5.5, 6.5, 6.5, 6.5, 5.5, 6.5, 6.5, 5.5,
            5.5, 6.5)
)
