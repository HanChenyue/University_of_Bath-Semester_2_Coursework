...
# ChatGPT used only to understand the concept of contrast in simple terms ONLY
...
As shown in the residuals vs fitted values plot, the variance of the residuals is fairly consistent (*no significant deviation on the datapoints observed*) with minor deviations. However, with only 4 replicates per treatment, it is difficult to truly assess the homogeneity of variance. Ideally, we would like to increase the number of replicates, to obtain a more accurate assessment of the homogeneity of variance to see if the spread is consistent across treatment levels. Therefore, for assumption #2, if we \textbf{ONLY} based on only 4 replicate per treatment, the assumption of homogeneity of variance is justified.
...
As shown in the QQ plot, the points at both tail ends falls approximately along the straight line  without significant deviations *(Observe the 3 points when theoretical quantiles are less than -1 and the 3 points when theoretical quantiles more than 1)*. This suggests that the residuals are normally distributed but problem is we do not have sufficient datapoint to really tell. Again, like assumption #2, the more datapoints, the more accurate our assessment of the normality of the residuals will be as 3 datapoints at each tail end is insufficient. Therefore, for assumption #3, if we \textbf{ONLY} based on observing the 3 datapoints per tail endpoints to check for significant deviation, the assumption of normality of the residuals is justified.
...
In this case, the data should be analysed using a randomised complete block design where the blocks would be the different batches of dough and the treatments would be the levels of the baking powder. This would allow us to account for the batch-to-batch variation by ensuring that all treatments are represented within each batch.
